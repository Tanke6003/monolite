...
import { postgresDialect, SequelizeDbPlugin, SqlGenericRepository } from "@monolite/data";
import type { EntityMetadata, IGenericRepository } from "@monolite/data";
import { readEnv, requireEnv, toInt } from "../../config/env";

/**
 * The persistence layer, for PostgreSQL.
 *
 ...
 *
 * PostgreSQL, SQL Server and MySQL share `SequelizeDbPlugin`: what actually
...
export function createDataSource(logger: ILogger, context: IRequestContext): DataSource {
  const plugin = new SequelizeDbPlugin(
    {
      engine: "postgres",
      host: readEnv("POSTGRES_HOST", "__dbHost__"),
      port: toInt(readEnv("POSTGRES_PORT"), __dbPort__, 1),
      username: readEnv("POSTGRES_USER", "__dbUser__"),
      ...
      database: readEnv("POSTGRES_DB", "__dbName__"),
      poolMin: toInt(readEnv("POSTGRES_POOL_MIN"), 0),
      poolMax: toInt(readEnv("POSTGRES_POOL_MAX"), 10, 1),
    },
    logger
  );
  ...
  return {
    ...
    repository: <T extends object>(metadata: EntityMetadata<T>) =>
      new SqlGenericRepository<T>(plugin, metadata, logger, postgresDialect, context),
  };
}
