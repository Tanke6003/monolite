...
import { mysqlDialect, SequelizeDbPlugin, SqlGenericRepository } from "@monolite/data";
import type { EntityMetadata, IGenericRepository } from "@monolite/data";
import { readEnv, requireEnv, toInt } from "../../config/env";

/**
 * The persistence layer, for MySQL / MariaDB.
 *
 ...
 *
 * MySQL, PostgreSQL and SQL Server share `SequelizeDbPlugin`: what actually
...
export function createDataSource(logger: ILogger, context: IRequestContext): DataSource {
  const plugin = new SequelizeDbPlugin(
    {
      engine: "mysql",
      host: readEnv("MYSQL_HOST", "__dbHost__"),
      port: toInt(readEnv("MYSQL_PORT"), __dbPort__, 1),
      username: readEnv("MYSQL_USER", "__dbUser__"),
      ...
      database: readEnv("MYSQL_DB", "__dbName__"),
      poolMin: toInt(readEnv("MYSQL_POOL_MIN"), 0),
      poolMax: toInt(readEnv("MYSQL_POOL_MAX"), 10, 1),
    },
    logger
  );
  ...
  return {
    ...
    repository: <T extends object>(metadata: EntityMetadata<T>) =>
      new SqlGenericRepository<T>(plugin, metadata, logger, mysqlDialect, context),
  };
}
