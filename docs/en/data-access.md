...
`defineEntity` already carries every fact a `CREATE TABLE` needs — the physical
names, the types, the key, whether it is generated, the soft-delete flag, the
timestamps, the audit columns and, since relations were declared, the foreign
keys. Until this existed none of it was used for that, so the schema was
maintained twice: once as metadata the code reads, once as SQL written by hand,
with nothing checking that the two agreed.

They disagree in ways that are hard to guess at. The mapping writes a boolean as
`1` and `0`, so a column declared `BOOLEAN` in PostgreSQL rejects every insert
the repository makes; the demo that hit it lost an afternoon before the column
became `SMALLINT`, which is what the generator emits.

```bash
npm run db:sql                      # to stdout, for DATA_SOURCE's engine
...
Every table is created before any of them is constrained. Not for tidiness: a
foreign key can point at a table declared later, and two tables that point at
each other have no valid ordering at all.

### What the mapping needs to say for it

Four optional fields on a column exist for the generator and for nothing else —
the repository has never needed to know how wide a column is, it binds values and
lets the engine check them. Leaving them off still maps, still queries and still
writes; it only generates a schema with the defaults.

| Field | Default |
| --- | --- |
| `length` | 255; `"max"` asks for the engine's unbounded text type |
| `precision` / `scale` | absent — a number is an integer |
| `nullable` | `true`, except the primary key |
| `unique` | `false`; for more than one column use `indexes` |
| `default` | none. Literal SQL, emitted verbatim — quote your own strings |

The permissive nullability default is deliberate: it is the only choice that
cannot break a table that already exists the first time the generated DDL is
compared against one written by hand.
...
deliberately not the live database. Reading the database would mean a migration
you cannot write without a connection to an environment, and one that came out
differently depending on which environment you were pointed at. The snapshot is
what the code said, which is the same for everybody who has the code.

**When nothing changed it writes nothing at all.** A tool that emits an empty
file every run teaches people to stop reading its output, and then the one that
mattered goes out unread.

**It cannot see a rename.** A column renamed in the metadata is, to a diff, one
column that vanished and another that appeared, and nothing distinguishes that
from an actual drop and add — the only thing that could, a stable identity per
column carried across the rename, does not exist in the mapping. So it emits
both and says so in the file. Turning the pair back into a `RENAME COLUMN` is a
two-line edit by the one person who knows it was a rename; guessing would
silently drop a populated column the day two unrelated changes landed together.

A property renamed over the same column produces nothing, because the snapshot
is keyed by column. Refactoring your code is not a migration.

Anything that destroys data — a dropped column, a dropped table — is written
**commented out**. Everything else here is additive or reversible; a generated
file that drops a table is a generated file somebody runs by accident.

### There is no runner, on purpose

The output is plain SQL for umzug, node-pg-migrate, Flyway or `psql < file` —
whatever this project already uses. Owning a runner means owning an
applied-migrations table, locking, ordering, rollback and every support question
about all four; and a toolkit you adopt one package at a time should not make
you switch migration tools in order to use its entity mapping.

Generating DDL at startup instead was considered and rejected for a shorter
reason: a process that alters a schema on boot is a process that alters
production on a bad deploy.
