...
A project generated by the CLI comes with `jest.config.js`, the same
source-not-dist mapping, and one smoke test that passes on the first run. Add to it
the same way:
...
`createApp`, register the error handling last or your errors escape as Express's
default HTML and the assertion above fails for a reason that has nothing to do with
validation.
