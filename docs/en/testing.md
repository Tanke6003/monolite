...
## Running it against every engine

The unit suite verifies the packages against an array and two doubles, and the
doubles are good ones — they evaluate the generated SQL rather than nodding at
it. What they cannot catch is the class of bug where the engine and the double
disagree, and that class shipped more than once: a transaction the repositories
did not join, a soft-delete flag the SQL driver never wrote, an `ON DELETE`
clause one engine has no word for, and a stored-procedure call the connector
could not make at all. Every one of them passed every unit test.

```bash
npm run engines:up          # five containers, waits until each is healthy
npm run test:integration
npm run engines:down        # and their volumes
```

`docker/integration/docker-compose.yml` brings up PostgreSQL, MySQL, SQL Server,
MongoDB and Oracle Free on ports chosen not to collide with anything you already
run. CI runs the same suite on every push, against the same five.

Narrow it while you work:

```bash
MONOLITE_IT_ENGINES=postgres,mysql npm run test:integration
MONOLITE_IT_VERBOSE=true npm run test:integration    # every statement, logged
```

Every connection setting has a `MONOLITE_IT_<ENGINE>_<FIELD>` override, so the
suite can be pointed at a server you did not start from that file.

### What it actually checks

- **The schema is the one this repository generates.** `emitSchema` writes it
  from the same metadata the repository reads, and the suite applies that. A
  hand-written schema would hide exactly the disagreement the generator exists
  to prevent — and it hid one for months: the mapping writes a boolean as `1`,
  which a `BOOLEAN` column rejects.
- **The shared contract, on all five.** `runGenericRepositoryContract` is the
  set of assertions every implementation must pass. It had only ever been run
  against the two that were already right.
- **Transactions across three tables**, committing and rolling back, including
  the case that matters most: a store injected at startup, inside a transaction
  it was never handed.
- **Row locks**, taken before the read that depends on them.
- **Stored procedures** through `executeRaw` — one that reads, one that writes,
  and one called inside a transaction that then rolls back, which is what proves
  the escape hatch runs on the transaction's connection and not on the pool's.

MongoDB skips the four SQL-only cases and says so; it has no procedures and no
foreign keys, and pretending otherwise would mean inventing a shape nobody
writes.

### It fails rather than skips

An engine named in `MONOLITE_IT_ENGINES` that is not reachable fails the run. A
suite that quietly verifies nothing is worse than one nobody runs, because CI
goes on reporting that the integration tests passed.
