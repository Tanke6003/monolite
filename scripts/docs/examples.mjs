 ...
 */
/**
 * Type arguments on a call whose callee is `any`.
 *
 * It can only ever be one of the stubs — `scope.repository<IAppointment>(...)`,
 * where the page never said what `scope` is. The example is right and the stub
 * is what cannot carry it, so reporting it would be reporting this file rather
 * than the documentation.
 */
const TYPE_ARGS_ON_UNTYPED_CALL = 2347;

function isOursToAnswerFor(diagnostic, message) {
  if (diagnostic.code === TYPE_ARGS_ON_UNTYPED_CALL) return false;
